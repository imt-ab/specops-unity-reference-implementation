# AI Workflow - Team Operating Model

_Last Updated: 2026-02-22_

This document defines the repository workflow for the hybrid AI model
(Claude, Junie, Codex) and the Rider Live Templates that support it.

This workflow is mandatory for feature work that changes specifications,
...
Testing is embedded inside slice completion and driven by acceptance
criteria. Rider test templates are enforcement tools, not default
ceremony steps.
...
All issues identified in `codexspec` must be resolved using the formal
`ISSUE_RESOLUTION_PROTOCOL.md` (Balanced Mode) before implementation
proceeds.
