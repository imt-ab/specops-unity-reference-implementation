...
Four eval definitions are installed under `.specops/evals/`, and `reference-architecture-example` has an instantiated derived feature state. The production Bootstrap and fresh-project Unity validation are complete. The qualified evidence subject remains A2 at `adddb2c91f7adae6133b384440d2c3f2020f1574`, and durable external evidence is staged at `release-evidence/v2.0.0-staging` commit `deb92a9b89624ea21d05bbac6ad6c4b34ad213b0`. Container B records the `v2.0.0` release lifecycle state; the annotated `v2.0.0` tag and GitHub Release publication remain separate Human Authority-controlled actions and have not yet occurred.
