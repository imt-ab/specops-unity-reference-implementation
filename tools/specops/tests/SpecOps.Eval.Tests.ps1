...
$producerPaths=@('tools/specops/Invoke-SpecOps.ps1','tools/specops/SpecOps.Core.psm1','tools/specops/SpecOps.Repository.psm1','tools/specops/SpecOps.Eval.psm1','tools/specops/SpecOps.Unity.psm1')
...
function New-CanonicalFixture{
 param([string]$Path,[string]$ProducerSourceRoot=$repoRoot)
 $null=Invoke-Git $repoRoot @('clone','--quiet','--no-local',$repoRoot,$Path)
 foreach($relative in $producerPaths){Copy-Item (Join-Path $ProducerSourceRoot $relative) (Join-Path $Path $relative) -Force}
 $null=Invoke-Git $Path @('config','user.email','specops-tests@example.invalid');$null=Invoke-Git $Path @('config','user.name','SpecOps Tests');$null=Invoke-Git $Path @('remote','remove','origin')
 $pending=@(Invoke-Git $Path @('status','--porcelain=v1','--untracked-files=all'))
 if($pending.Count-gt0){Commit-Repo $Path 'canonical E8C3 fixture'}
}
...
function New-UnityExecution{
 param(
  [string[]]$RequiredNames,
  [string]$ActualVersion='6000.5.8f1',[string]$ActualRevision='5cb7df797b7d',
  [string]$ExpectedVersion='6000.5.8f1',[string]$ExpectedRevision='5cb7df797b7d',
  [bool]$CompilationCompleted=$true,[int]$CompilationErrors=0,[int]$Warnings=0,
  [string]$AssemblyName='InfiniteMonkey.EditModeTests',[int]$Failed=0,[int]$Inconclusive=0,[int]$NotExecuted=0,
  $ExitCode=0,[object[]]$ChangedEntries=@(),[string[]]$GeneratedPaths=@(),
  [string]$PackagesLockStatus='Preserved',[bool]$CleanupSucceeded=$true
 )
 return [pscustomobject]@{
  Runtime=[pscustomobject]@{EditorVersion=$ActualVersion;EditorRevision=$ActualRevision}
  ExpectedRuntime=[pscustomobject]@{EditorVersion=$ExpectedVersion;EditorRevision=$ExpectedRevision}
  Compilation=[pscustomobject]@{Completed=$CompilationCompleted;Errors=$CompilationErrors;Warnings=$Warnings;WarningObservability='COMPILATION_WARNING_OBSERVABILITY_UNRESOLVED'}
  NUnit3=[pscustomobject]@{AssemblyName=$AssemblyName;FullyQualifiedTestNames=@($RequiredNames);Counts=[pscustomobject]@{Failed=$Failed;Inconclusive=$Inconclusive;NotExecuted=$NotExecuted}}
  Process=[pscustomobject]@{ExitCode=$ExitCode}
  ChangedEntries=@($ChangedEntries);GeneratedPaths=@($GeneratedPaths)
  PackagesLock=[pscustomobject]@{Status=$PackagesLockStatus;ExactBytePreserved=($PackagesLockStatus-eq'Preserved')}
  Cleanup=[pscustomobject]@{Attempted=$true;Succeeded=$CleanupSucceeded;Diagnostic=$(if($CleanupSucceeded){$null}else{'fixture cleanup failure'})}
 }
}
function Invoke-UnityMappedCheck{param($Definition,$Execution,[string]$Id)$check=@($Definition.checks|Where-Object{$_.id-eq$Id})[0];return & (Get-Module SpecOps.Eval) {param($c,$e)Invoke-SpecOpsUnityDefinitionCheck $c $e} $check $Execution}
...
try{
 ...
 Test-Case 'canonical fixture commits and binds all five current producer files'{foreach($relative in $producerPaths){$expected=@(Invoke-Git $repoRoot @('hash-object','--no-filters','--',(Join-Path $repoRoot $relative)))[0];$actual=@(Invoke-Git $clean @('rev-parse',"HEAD:$relative"))[0];Assert-Equal $expected $actual "Fixture producer binding: $relative"};Assert-Equal 0 @(Invoke-Git $clean @('status','--porcelain=v1','--untracked-files=all')).Count 'Producer fixture is dirty.'}
 Test-Case 'canonical fixture commits differing producer bytes exactly'{$producerSource=Join-Path $tempBase 'producer-source';foreach($relative in $producerPaths){$destination=Join-Path $producerSource $relative;$null=[IO.Directory]::CreateDirectory([IO.Path]::GetDirectoryName($destination));Copy-Item (Join-Path $repoRoot $relative) $destination -Force};$changedRelative='tools/specops/SpecOps.Eval.psm1';$changedSource=Join-Path $producerSource $changedRelative;[IO.File]::AppendAllText($changedSource,"`n# fixture producer-byte delta`n",[Text.UTF8Encoding]::new($false));$changedFixture=Join-Path $tempBase 'changed-producer';New-CanonicalFixture $changedFixture $producerSource;$baselineHead=@(Invoke-Git $repoRoot @('rev-parse','HEAD'))[0];$fixtureHead=@(Invoke-Git $changedFixture @('rev-parse','HEAD'))[0];Assert-True (-not[string]::Equals($baselineHead,$fixtureHead,[StringComparison]::Ordinal)) 'Differing producer bytes did not create a fixture commit.';$expectedBlob=@(Invoke-Git $changedFixture @('hash-object','--no-filters','--',$changedSource))[0];$actualBlob=@(Invoke-Git $changedFixture @('rev-parse',"HEAD:$changedRelative"))[0];Assert-Equal $expectedBlob $actualBlob 'Fixture did not retain the copied producer bytes.';Assert-Equal 0 @(Invoke-Git $changedFixture @('status','--porcelain=v1','--untracked-files=all')).Count 'Changed producer fixture is dirty.'}
 ...
 $requiredNames=[string[]]@((@($unityDefinition.checks|Where-Object id -eq 'editmode-required-tests-discovered')[0]).passCondition.value.requiredItems)
 $passingExecution=New-UnityExecution -RequiredNames $requiredNames
 Test-Case 'producer implementation set is exactly the approved five files'{$actual=&(Get-Module SpecOps.Eval){$script:ProducerPaths};Assert-Equal 5 @($actual).Count 'Producer path count.';for($i=0;$i-lt$producerPaths.Count;$i++){Assert-Equal $producerPaths[$i] $actual[$i] 'Producer path/order.'}}
 Test-Case 'supplied Unity seam executes seven checks instead of adapter-not-installed defaults'{$execution=$passingExecution;$unityAdapter={param($context,$definition)$execution}.GetNewClosure();$result=Invoke-SpecOpsEvaluation -RepositoryAdapter (New-SpecOpsGitRepositoryAdapter $clean) -DefinitionId 'unity-editmode-validation' -UnityExecutionAdapter $unityAdapter;Assert-Equal PASS $result.overallResult 'Supplied Unity aggregate.';Assert-Equal 7 @($result.checkResults|Where-Object{$_.executed}).Count 'Executed Unity count.';Assert-Equal 0 @($result.checkResults|Where-Object{$_.Contains('notExecutedReason')}).Count 'Unexpected non-execution.'}
 Test-Case 'version match passes exact version and revision'{$check=Invoke-UnityMappedCheck $unityDefinition $passingExecution 'unity-version-match';Assert-Equal PASS $check.result 'Version match.'}
 Test-Case 'version mismatch fails ordinal identity'{$execution=New-UnityExecution -RequiredNames $requiredNames -ActualRevision 'wrong-revision';$check=Invoke-UnityMappedCheck $unityDefinition $execution 'unity-version-match';Assert-Equal FAIL $check.result 'Version mismatch.'}
 Test-Case 'compilation requires completion and zero errors while warnings do not fail'{$warnings=New-UnityExecution -RequiredNames $requiredNames -Warnings 17;$incomplete=New-UnityExecution -RequiredNames $requiredNames -CompilationCompleted $false;$errors=New-UnityExecution -RequiredNames $requiredNames -CompilationErrors 1;Assert-Equal PASS (Invoke-UnityMappedCheck $unityDefinition $warnings 'unity-compilation-success').result 'Warnings failed compilation.';Assert-Equal FAIL (Invoke-UnityMappedCheck $unityDefinition $incomplete 'unity-compilation-success').result 'Incomplete compilation passed.';Assert-Equal FAIL (Invoke-UnityMappedCheck $unityDefinition $errors 'unity-compilation-success').result 'Compilation errors passed.'}
 Test-Case 'established compilation failure is a legitimate failing eval observation without fabricated NUnit facts'{$execution=New-UnityExecution -RequiredNames $requiredNames -CompilationErrors 1 -ExitCode 1;$execution.NUnit3=$null;$unityAdapter={param($context,$definition)$execution}.GetNewClosure();$result=Invoke-SpecOpsEvaluation -RepositoryAdapter (New-SpecOpsGitRepositoryAdapter $clean) -DefinitionId 'unity-editmode-validation' -UnityExecutionAdapter $unityAdapter;Assert-Equal FAIL $result.overallResult 'Compilation failure aggregate.';Assert-Equal FAIL (@($result.checkResults|Where-Object id -eq 'unity-compilation-success')[0]).result 'Compilation failure passed.';$suiteChecks=@($result.checkResults|Where-Object{$_.id-in@('editmode-suite-executed','editmode-required-tests-discovered','editmode-zero-failures')});Assert-Equal 3 $suiteChecks.Count 'Suite check count.';foreach($check in $suiteChecks){Assert-Equal NOT_EXECUTED $check.result 'Absent NUnit fact was fabricated.';Assert-Equal UNITY_COMPILATION_FAILED $check.notExecutedReason 'Compilation-failure reason.'}}
 Test-Case 'suite execution requires exact complete assembly identity'{$wrong=New-UnityExecution -RequiredNames $requiredNames -AssemblyName 'Other.EditModeTests';Assert-Equal PASS (Invoke-UnityMappedCheck $unityDefinition $passingExecution 'editmode-suite-executed').result 'Expected suite failed.';Assert-Equal FAIL (Invoke-UnityMappedCheck $unityDefinition $wrong 'editmode-suite-executed').result 'Wrong suite passed.'}
 Test-Case 'required exact fourteen-test inventory is consumed from definition and normalized NUnit3 facts'{Assert-Equal 14 $requiredNames.Count 'Definition required-test count.';Assert-Equal PASS (Invoke-UnityMappedCheck $unityDefinition $passingExecution 'editmode-required-tests-discovered').result 'Exact inventory failed.';$missing=New-UnityExecution -RequiredNames ([string[]]$requiredNames[0..12]);Assert-Equal FAIL (Invoke-UnityMappedCheck $unityDefinition $missing 'editmode-required-tests-discovered').result 'Missing required identity passed.'}
 Test-Case 'zero failures requires failed inconclusive and notExecuted all zero'{Assert-Equal PASS (Invoke-UnityMappedCheck $unityDefinition $passingExecution 'editmode-zero-failures').result 'Zero counts failed.';foreach($case in @(@{Failed=1;Inconclusive=0;NotExecuted=0},@{Failed=0;Inconclusive=1;NotExecuted=0},@{Failed=0;Inconclusive=0;NotExecuted=1})){$execution=New-UnityExecution -RequiredNames $requiredNames -Failed $case.Failed -Inconclusive $case.Inconclusive -NotExecuted $case.NotExecuted;Assert-Equal FAIL (Invoke-UnityMappedCheck $unityDefinition $execution 'editmode-zero-failures').result 'Nonzero terminal count passed.'}}
 Test-Case 'Unity exit requires exact integer zero'{$nonzero=New-UnityExecution -RequiredNames $requiredNames -ExitCode 1;Assert-Equal PASS (Invoke-UnityMappedCheck $unityDefinition $passingExecution 'unity-exit-success').result 'Exit zero failed.';Assert-Equal FAIL (Invoke-UnityMappedCheck $unityDefinition $nonzero 'unity-exit-success').result 'Nonzero exit passed.'}
 Test-Case 'tracked state passes with zero changed entries and generated-only paths'{$generated=New-UnityExecution -RequiredNames $requiredNames -GeneratedPaths @('Library/cache.bin','Temp/state');Assert-Equal PASS (Invoke-UnityMappedCheck $unityDefinition $generated 'unity-tracked-state-preserved').result 'Generated-only paths failed.'}
 Test-Case 'tracked state fails Modified and Missing entries'{foreach($change in @('Modified','Missing')){$execution=New-UnityExecution -RequiredNames $requiredNames -ChangedEntries @([pscustomobject]@{Path='ProjectSettings/ProjectVersion.txt';Change=$change});Assert-Equal FAIL (Invoke-UnityMappedCheck $unityDefinition $execution 'unity-tracked-state-preserved').result "$change entry passed."}}
 Test-Case 'packages-lock modification is a generic changed entry with no special rule'{$execution=New-UnityExecution -RequiredNames $requiredNames -PackagesLockStatus 'Modified' -ChangedEntries @([pscustomobject]@{Path='Packages/packages-lock.json';Change='Modified'});$check=Invoke-UnityMappedCheck $unityDefinition $execution 'unity-tracked-state-preserved';Assert-Equal FAIL $check.result 'Modified packages-lock passed.';Assert-True ((@($check.observations)-join';').Contains('specialRule=false',[StringComparison]::Ordinal)) 'Special-rule diagnostic missing.'}
 Test-Case 'cleanup outcome remains separate from seven check facts'{$execution=New-UnityExecution -RequiredNames $requiredNames -CleanupSucceeded $false;foreach($check in $unityDefinition.checks){Assert-True ((Invoke-UnityMappedCheck $unityDefinition $execution $check.id).result-ne'INCONCLUSIVE') 'Cleanup altered a check fact.'};$unityAdapter={param($context,$definition)$execution}.GetNewClosure();Assert-Rejection {Invoke-SpecOpsEvaluation -RepositoryAdapter (New-SpecOpsGitRepositoryAdapter $clean) -DefinitionId 'unity-editmode-validation' -UnityExecutionAdapter $unityAdapter} 'UNITY_CLEANUP_FAILED' 4}
 Test-Case 'Unity observation parser failure follows deterministic Eval error semantics'{$failingAdapter={param($context,$definition)$exception=[InvalidOperationException]::new('fixture parser failure');$exception.Data['SpecOpsRejectionClass']='UNITY_NUNIT_XML_INVALID';throw $exception};Assert-Rejection {Invoke-SpecOpsEvaluation -RepositoryAdapter (New-SpecOpsGitRepositoryAdapter $clean) -DefinitionId 'unity-editmode-validation' -UnityExecutionAdapter $failingAdapter} 'UNITY_NUNIT_XML_INVALID' 4}
 Test-Case 'dirty Unity subject is rejected before supplied execution seam'{$state=[pscustomobject]@{Called=$false};Write-Utf8 (Join-Path $clean 'nonignored-e8c4d2.txt') 'dirty';try{$unityAdapter={param($context,$definition)$state.Called=$true;$passingExecution}.GetNewClosure();Assert-Rejection {Invoke-SpecOpsEvaluation -RepositoryAdapter (New-SpecOpsGitRepositoryAdapter $clean) -DefinitionId 'unity-editmode-validation' -UnityExecutionAdapter $unityAdapter} 'SUBJECT_NOT_CLEAN' 2;Assert-Equal False $state.Called 'Unity seam was called for dirty subject.'}finally{[IO.File]::Delete((Join-Path $clean 'nonignored-e8c4d2.txt'))}}
 Test-Case 'CLI routes Unity definition to production adapter without executing it in this test'{$source=Get-Content -Raw (Join-Path $clean 'tools/specops/Invoke-SpecOps.ps1');Assert-True ($source.Contains('Get-SpecOpsUnityProductionExecutionAdapter',[StringComparison]::Ordinal)) 'CLI production adapter routing absent.'}
 Test-Case 'production adapter validates registry and Git package capabilities and applies acquisition guard before launch'{$source=Get-Content -Raw (Join-Path $repoRoot 'tools/specops/SpecOps.Eval.psm1');$registryIndex=$source.IndexOf('Get-SpecOpsUnityRegistryPackageCacheCapabilities -ProjectRoot $materialization.ProjectRoot',[StringComparison]::Ordinal);$capabilityIndex=$source.IndexOf('Initialize-SpecOpsUnityOfflinePackageCapability -ProjectRoot $materialization.ProjectRoot',[StringComparison]::Ordinal);$guardIndex=$source.IndexOf('Get-SpecOpsUnityPackageAcquisitionGuard',[StringComparison]::Ordinal);$processIndex=$source.IndexOf('Invoke-SpecOpsControlledProcess -FilePath $selection.Candidate.Path',[StringComparison]::Ordinal);Assert-True ($registryIndex-ge0-and$capabilityIndex-gt$registryIndex-and$guardIndex-gt$capabilityIndex-and$processIndex-gt$guardIndex) 'Registry/Git package preflight and guard are not fail-closed before Unity launch.';Assert-True ($source.IndexOf('-EnvironmentVariables $packageAcquisitionGuard',[StringComparison]::Ordinal)-gt$processIndex) 'Package acquisition guard is not attached to Unity process.'}
 Test-Case 'Core remains unchanged and contains no D2 module coupling'{Assert-Equal 0 @(Invoke-Git $repoRoot @('diff','--name-only','--','tools/specops/SpecOps.Core.psm1')).Count 'Core was modified.';$source=Get-Content -Raw (Join-Path $repoRoot 'tools/specops/SpecOps.Core.psm1');Assert-True (-not$source.Contains('SpecOps.Unity',[StringComparison]::Ordinal)-and-not$source.Contains('SpecOps.Repository',[StringComparison]::Ordinal)) 'Core gained module coupling.'}
